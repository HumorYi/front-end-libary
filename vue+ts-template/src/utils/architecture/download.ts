...
export default class Download implements InterfaceDownload {
  ...
  getFilenameSuffix(contentDisposition: string): string {
    const oldIE = contentDisposition.match(/filename=(.*)/)
    const supportEncode = contentDisposition.match(/filename\*=(.*)/)

    if (supportEncode) {
      return decodeURIComponent(supportEncode[1].substring(6))
    }

    if (oldIE) {
      return decodeURIComponent(oldIE[1])
    }
