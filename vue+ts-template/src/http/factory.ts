...
const getPendingRequestKey = (config: AxiosRequestConfig): string => {
  return config['similar'] || config['url'] + '' + config['method']
}
  ...
  instance.interceptors.response.use(
    ...
    (err: any): any => {
      ...
      return Promise.reject(
        errConfig['selfCancel'] ? 'cancel request' : errConfig
      )
    }
  )
}
...
// 多个 http 共享一个 timer，避免多个实例时出现多个 loading
const shareTimer = new Timer()
